...
load(paste("temp_",strsplit(args[1],split="_3tail")[[1]][1],".RData",sep=""))
...
all_gene_all_transcript_tes <- read.table("gencode_hg38_all_gene_all_transcript_tes.bed",sep = "\t")
...
tc <- data.frame(tc[,c(1,2,3,4,5,6,7,8,11)],peak_length=(tc[,4]-tc[,3]),dominant_tes_RPM.smart2_seq_RPM=tc[,8]/tc[,10],tc[,c(14,15,13)])
tc[is.na(tc)] <- 0
...
write.csv(tc,paste('tc_',strsplit(args[1],split = "_3tail")[[1]][1],"_3tail.csv",sep = ""),quote = F,row.names = F)

write.table(tc[,c(2,3,4,1,6,5)],paste("temp_",strsplit(args[1],split="_3tail")[[1]][1],"_tes.bed",sep=""),quote = FALSE,row.names = FALSE,col.names = FALSE,sep = "\t")
