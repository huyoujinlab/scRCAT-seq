...
load(paste("temp_",strsplit(args[1],split="_5cap")[[1]][1],".RData",sep=""))
...
a <- paste('tc <- merge(',paste('tc_',strsplit(args[1],split = "_domin")[[1]][1],sep = ''),',dominant_tss_in_gene,by=c("chr","start","end","strand"))',sep = "")
eval(parse(text=a))
print(a)
...
all_gene_all_transcript_tss <- read.table("gencode_hg38_all_gene_all_transcript_tss.bed",sep = "\t")
...
tc <- data.frame(tc[,c(1,2,3,4,5,6,7,8,11)],peak_length=(tc[,4]-tc[,3]),dominant_tss_RPM.smart2_seq_RPM=tc[,8]/tc[,10],tc[,c(14,15,13)])
tc[is.na(tc)] <- 0
...
write.csv(tc,paste('tc_',strsplit(args[1],split = "_5cap")[[1]][1],"_5cap.csv",sep = ""),quote = F,row.names = F)

write.table(tc[,c(2,3,4,1,6,5)],paste("temp_",strsplit(args[1],split="_5cap")[[1]][1],"_tss.bed",sep=""),quote = FALSE,row.names = FALSE,col.names = FALSE,sep = "\t")
