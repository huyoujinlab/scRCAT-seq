...
mkdir outdir/three_pirme/mapping_outdir/
mkdir outdir/three_pirme/final_out/
mkdir outdir/three_pirme/annote/
mkdir outdir/three_pirme/collapse/
...
do
        STAR --runThreadN 24 --genomeDir ${star_dir} --genomeLoad LoadAndKeep --readFilesIn outdir/three_pirme/3tail_read_with_tag_other_strand_withA10_remain_A5/${i} --outFileNamePrefix outdir/three_pirme/mapping_outdir/${i}_ --outSAMtype SAM --outFilterMultimapNmax 1 --outFilterScoreMinOverLread 0.6 --outFilterMatchNminOverLread 0.6
done


#Output files are stored in `outdir/three_pirme/mapping_outdir/`.


## 5. Remove useless file


for i in `ls outdir/three_pirme/mapping_outdir |grep "sam"|grep "R1"`
do
### The first read count
        a=$(wc -l outdir/three_pirme/mapping_outdir/${i}|awk '{print $1}')
        echo ${a}

### The second read count
        b=$(wc -l outdir/three_pirme/mapping_outdir/${i%%R1*}R2.fastq-common.out_withA10_remain_A5_Aligned.out.sam|awk '{print $1}')
        echo ${b}

### Remove useless files
        if [ ${a} -gt ${b} ]; then
                rm outdir/three_pirme/mapping_outdir/${i%%R1*}R2*
        else
                rm outdir/three_pirme/mapping_outdir/${i%%R1*}R1*
        fi
done
...
for i in `ls outdir/three_pirme/mapping_outdir | grep "sam$"`
do
### Add header and convert to bam
        samtools view -b -T $fa_file} outdir/three_pirme/mapping_outdir/${i} | samtools view -b > outdir/three_pirme/final_out/${i}_add_header.bam
...
for i in `ls outdir/three_pirme/final_out | grep "sorted_remove_trRNA.bed$"`
do

        if [ ${i#*.R} == "1.fastq-common.out_withA10_remain_A5_Aligned.out.sam_add_header_sorted_remove_trRNA.bed" ]; then
                python2 annotate_UMI_v1.py -N 18 -n 8 -F ${fq_dir}/${i%%.R*}.R2.fastq -ID outdir/three_pirme/final_out/${i} -O outdir/three_pirme/annote/${i}.annote
        fi
        if [ ${i#*.R} == "2.fastq-common.out_withA10_remain_A5_Aligned.out.sam_add_header_sorted_remove_trRNA.bed" ]; then
                python2 annotate_UMI_v1.py -N 18 -n 8 -F ${fq_dir}/${i%%.R*}.R1.fastq -ID outdir/three_pirme/final_out/${i} -O outdir/three_pirme/annote/${i}.annote
        fi
done
...
for i in `ls outdir/three_pirme/annote | grep "annote$"`
do
        Rscript collapse_UMI_5.R  outdir/three_pirme/annote/${i}
done

mv outdir/three_pirme/annote/*collapse outdir/three_pirme/collapse/


## 10. collapse 6 and change name

for i in `ls outdir/three_pirme/collapse | grep "collapse$"`
do
        awk '{FS=" "}{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6}' outdir/three_pirme/collapse/${i} > outdir/three_pirme/collapse/${i}.6
        mv outdir/three_pirme/collapse/${i}.6 outdir/three_pirme/collapse/${i%%L3*}3_TKD.bed
done
...
for i in `ls outdir/three_pirme/collapse | grep "TKD.bed$"`
do
        bed_file=outdir/three_pirme/collapse/${i}
done
...
## Call peak 
Rscript CAGE_dominant_TES.R ${bed_file} ${threshold}


## Extract peak in gene region
bedtools intersect -s -a ${prefix}_3tail_dominant_tes.bed -b gencode_hg38_all_gene_genebody_and_downstream2k.bed -wa -wb > ${prefix}_3tail_dominant_tes_genebody_and_downstream2k.bed


## Calculate slope, correaltion, ect.
Rscript cal_slope_TES.R ${prefix}_3tail_dominant_tes_genebody_and_downstream2k.bed ${prefix}_3tail_dominant_tes.bed

echo ${prefix}
## Compare to polydb
bedtools intersect -s -a temp_${prefix}_tes.bed -b ${PolyA_db} -wa -wb > temp_${prefix}_tes_in_polydb.bed
Rscript polydb.R tc_${prefix}_3tail.csv


## Add motif information
python find_motif_re_TES.py ${fa_file} tc_${prefix}_3tail_new.csv tc_${prefix}_3tail_new_new.csv
...
## internal
sh 3.sh tc_${prefix}_3tail_final.csv ${fa_file} gencode_hg38_all_gene.bed 
