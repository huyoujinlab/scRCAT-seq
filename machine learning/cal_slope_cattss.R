...
all_gene_all_transcript_tss <- read.table("gencode_mm10_all_gene_all_transcript_tss.bed",sep = "\t")


for(j in 1:nrow(tc)) {
  temp <- all_gene_all_transcript_tss[all_gene_all_transcript_tss[,1]==tc[j,2] & all_gene_all_transcript_tss[,6]==tc[j,5] & abs(all_gene_all_transcript_tss[,3]-tc[j,7])<=100,]
