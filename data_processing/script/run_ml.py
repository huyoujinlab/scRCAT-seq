...
parser.add_argument('--num', '-n', default=3, help='Keep the peak with the value greater than or equal to the threshold. default = 3')
parser.add_argument('--prefix', '-p', default="output", help='The prefix of the output folder, default=output')
parser.add_argument('--model', '-m',  default=None, help='Pre-trained model path, which is used for prediction of other dataset, default = None')
...
if types == "tes":
    features_read_distribution = tmp[[5,7,8]]
        ...
        if type(self.features) == list:
            ...
            for feature in self.features:
                ...
                if feature == "peak_width":
                    ...
                    data[return_feature] = (data[[feature]].values - peak_width_min) / (peak_width_max - peak_width_min)
...
#             data = data.loc[data[colname] == data["annotated_peak"]]
    ...
    def prediction(self, test=None):
        ...
        accuracy_BM = metrics.accuracy_score(np.ones(len(test_label), dtype="int64"), test_label)
        ...
        return [train_f1_score, test_f1_score, train_accuracy, test_accuracy, train_recall, test_recall, train_precision, test_precision, train_roc, test_roc, accuracy_BM]
    ...
    def model(self, model=None, test=None):
        ...
        output_name = os.path.join(self.OutputDir, test.split("/")[-1].split(".")[0] + "_" + self.method + "_prediction.csv")
        output.to_csv(output_name, index = False)
        return [test_f1_score, test_accuracy, test_recall, test_precision, test_roc, accuracy_BM]

if __name__ == "__main__":
    dicts = {}
    l = 0
    methods = ["rf"] # can add knn, rf, lr, svm and vote methods
        
    feature = "all"
    features = np.append(features_read_distribution, np.append(features_motif, features_internal)).tolist()
    for method in methods:
            l += 1
            ml = ML(features=features, train =train, test =output_file_gene, method=method, OutputDir=os.path.join(outputdir, "model"))
            if args.model == None:
                ml.training(CList=[1])
                ...
                ModelFiles = args.model
                results = ml.model(model=ModelFiles)
                results_intergenic = ml.model(model=ModelFiles, test=output_file_intergenic)
                dicts[l] = results + [method, feature, types] 

    if args.model == None:
        df = pd.DataFrame.from_dict(dicts, orient="index", columns=["train_f1_score", "test_f1_score", "train_accuracy", "test_accuracy", "train_recall", "test_recall", "train_precision", "test_precision", "train_roc", "test_roc", "accuracy_BM", "method", "feature", "type"])
        df.to_csv(os.path.join(outputdir, "metrics.csv"), index=False)
    else:
        df = pd.DataFrame.from_dict(dicts, orient="index", columns=["test_f1_score", "test_accuracy", "test_recall", "test_precision", "test_roc", "accuracy_BM", "method", "feature", "type"])
        df.to_csv(os.path.join(outputdir, "metrics.csv"), index=False)

    # print(os.path.join(outputdir, "metrics.csv"))
    ...
    # p=ggplot(df, aes(x="type", y="value", fill = "method")) + geom_bar(stat="identity", position="dodge") + facet_grid([".","variable"]) + theme_light()
